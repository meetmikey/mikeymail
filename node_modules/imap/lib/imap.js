...
ImapConnection.prototype.connect = function(loginCb) {
  ...
  function ondata(b) {
    ...
    if (indata.line[0] === '*') { // Untagged server response
      ...
      if (line[0] === '+' && requests[0] && requests[0].cmd === 'AUTHENTICATE' && 'xoauth2' in self._options) {
        line = indata.line = new Buffer(line.slice(2), 'base64').toString('ascii');
        console.log (line)
        // console.assert(line === '{"status":"400","schemes":"Bearer","scope":"https://mail.google.com/"}');
      }
      indata.literals = [];
      indata.line = undefined;
      indata.temp = undefined;
      ...
      if (typeof requests[0].callback === 'function') {
        ...
        if (line[0] === '+') {
          if (requests[0].cmd !== 'APPEND') {
            ...
            console.error ('req 0', requests[0]);
          ...
          // m[3]: message
          console.error ('m', m);
          console.error ('m1', m[1]);
          console.error ('m2', m[2]);
          console.error ('m3', m[3]);
          err = new Error(m[3]);
          err.level = 'protocol';
          err.type = 'failure';
          err.code = m[2];
