...
exports.getMoreHeadersForResume = function (argDict, callback) {
  ...
  // determine in db the earliest gmDate that we have
  daemonUtils.getMinMailDateInDbForUser (argDict.userId, function (err, minGmDateInDb) {
    if (err) {
      callback (err);
    } else {

      console.log (minGmDateInDb);
      console.log (argDict.minDateAccount.getTime());

      if (argDict.minDateAccount.getTime() < minGmDateInDb) {
        // get id's of mail between first parameter and second parameter
        var after = Math.floor(argDict.minDateAccount.getTime()/1000);
        var before = Math.floor (minGmDateInDb/1000);
        imapRetrieve.getIdsOfMessagesInDateRange (argDict.myConnection, after, before, 
          function (err, uids) {
            if (err) {
              callback (err);
            } else if (uids.length == 0) {
              winston.doWarn ('resume, not uid length');
              callback(null, argDict);
            } else {
              // TODO: break up uids into batches
              imapRetrieve.getHeaders (argDict, null, null, uids, callback);
            }
          });
      } else {
        callback(null, argDict);
...
exports.getHeaders = function (argDict, minUid, maxUid, uidArray, callback) {
  ...
  console.log ('UID ARRAY', argDict.uidArray);
...
exports.getIdsOfMessagesWithAttachments = function (imapConn, minUid, maxUid, uidArray, callback) {
  ...
  imapConn.search([ ['X-GM-RAW', 'has:attachment'], ['UID', uidQuery]], function(err, results) {
    console.log ('hastattachemnt ids', results)
    callback (err, results)
  });
