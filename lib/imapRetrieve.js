...
exports.getHeadersInBatches = function (imapConn, userId, mailboxId, minUid, maxUid, callback) {
...
/*
  ...
  for (var i = 0, i < numIntervals; i ++) {
    ranges.push {
      minUid : i + minUid + i*constants.HEADER_BATCH_SIZE, 
      maxUid : Math.min((i+1)*constants.HEADER_BATCH_SIZE, maxUid)
    }

    parallelFunctions.push (function (parallelCb) {
      getHeaders (imapConn, userId, mailboxId, minUid, maxUid, parallelCb)
    });
  }

  async.parallel(parallelFunctions, callback);

  var currentMinUid = constants.HEADER_BATCH_SIZE;
  //  imapRetrieve.getHeaders(imapConn, userId, mailboxId, minUid, maxUid, 
  */
