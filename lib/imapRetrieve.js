...
exports.checkLabelIsInvalid = function (mailObject, folderNames) {  
  ...
  if (folderNames['TRASH']) {
    skipLabels.push (folderNames['TRASH'].toLowerCase());
  }
  
  if (folderNames['DRAFTS']) {
    skipLabels.push (folderNames['DRAFTS'].toLowerCase());
  }

  if (folderNames ['SPAM']) {
    skipLabels.push (folderNames['SPAM'].toLowerCase());
  }
...
exports.fetchHeaderToStayAlive = function (imapConn, userId) {
  winston.info ('fetchHeaderToStayAlive fetch', {userId : userId})
  ...
  try {
    imapConn.fetch('1:1',
      ...
      { headers: ['message-id', 'from', 'to', 'cc', 'bcc'],
        cb: function(fetch) {
          fetch.on ('end', function () {
            winston.info ('fetchHeaderToStayAlive fetch complete', {userId : userId})
          });
        }
      }, 
      function(err) {
        if (err) {
          winstin.doError ('Error : fetchHeaderToStayAlive', {error : err});
        }
      });
