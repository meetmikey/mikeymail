...
exports.getHeaders = function (imapConn, userId, mailboxId, minUid, maxUid, onboardingStateId, callback) {
  ...
  if (!minUid || !maxUid || minUid > maxUid) {
    return callback (winston.makeError ('getHeaders validation error: minUid, maxUid invalid', {userId : userId, stateId : onboardingStateId, minUid : minUid, maxUid : maxUid}));
  }
...
exports.getMarketingTextIds = function (imapConn, minUid, maxUid, callback) {  var uidRange = getUidRangeString(minUid, maxUid)
...
exports.fetchMsgBodies = function (imapConn, uidArray, userId, isUpdate, getAllMessagesCallback) {
  ...
  if (constants.USE_AZURE) {
    var cloudDirectory = conf.azure.blobFolders.rawEmail + '/' + userId;
  }
  else {
    var cloudDirectory = conf.aws.s3Folders.rawEmail + '/' + userId;
  }
