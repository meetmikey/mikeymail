...
exports.getMoreHeadersForResume = function (argDict, callback) {
  ...
  if (argDict.isPremium) {
    ...
    var uidLen = argDict.uidArray.length;
    var numBatches = 1;
    ...
    for (var i=0; i<numBatches; i++) {
      if (i == numBatches - 1) {
        asyncFunctionArguments.push ({
          'uids' : argDict.uidArray.slice (i * mikeyMailConstants.RESUME_BATCH_SIZE, uidLen)
        });
      } else {
        asyncFunctionArguments.push ({
          'uids' : argDict.uidArray.slice (i * mikeyMailConstants.RESUME_BATCH_SIZE, (i+1) * mikeyMailConstants.RESUME_BATCH_SIZE)
        });
      }
    }
...
exports.getHeaders = function (argDict, minUid, maxUid, uidArray, callback) {
  ...
  fetch.on ('error', function (err) {
    winston.doWarn ('FETCH ERROR', {msg : err.message, stack : err.stack});
  });
...
exports.getIdsOfMessagesInDateRange = function (imapConn, minDate, maxDate, callback) {
  ...
  imapConn.search([ ['X-GM-RAW', searchString]], function(err, results) {
    if (err) {
      callback (winston.makeError ('imap error getting ids of messages in date range', { err : err}));
    } else {
      callback (null, results);
    }
  });
