    ...
    for (var i = 0; i < numIntervals; i++) {
      ...
      if (intervalsToSkip && intervalsToSkip.length) {
        ...
        intervalsToSkip.forEach (function (interval) {
          console.log ('interval minUid', interval.minUid);
          console.log ('interval maxUid', interval.maxUid)
          //TODO: be less strict
        ...
        if (pushToAsync) {
          console.log ('not skipping the following')
          console.log ('push to async minUid', minUidBatch);
          console.log ('push to async minUid', maxUidBatch);
      ...
      { headers: ['message-id', 'from', 'to', 'cc', 'bcc'],
        cb: function(fetch) {
          ...
          fetch.on ('end', function () {
            ...
            if (docs.length == 0) {
              callback (null, {minUid : minUid, maxUid : maxUid});
              return;
            }
