...
exports.uploadFileStreamToS3 = function (stream, awsPath, headers, userId, uid, isUpdate) {

  knoxClient.putStream(stream, awsPath, headers, 
    function(err, res){     
      if (err) {
        console.error (err)
        winston.doError ('error uploading file', {'error' : err, 'aws path' : awsPath})
      }
      else{
        if (res.statusCode !== 200) {
          winston.doError ('Error: non 200 status code', {'statusCode' : res.statusCode, 'aws path' : awsPath})
        }
        else {
