  ...
  sqsConnect.pollMailDownloadQueue(function (message, pollQueueCallback) {
    ...
    UserOnboardingStateModel.findOne ({'userId' : userInfo._id}, function (err, foundState) {
      ...
      if (err) {
        ...
        pollQueueCallback (winston.makeError ('User onboarding previously started and threshold not high enough to restart', {userEmail : userInfo.email}));
      }
    });
...
exports.scrapeMailbox = function (xoauth2gen, userInfo, onboardingState, isFirstAttempt, pollQueueCallback) {
  ...
  xoauth2gen.getToken(function(err, token) {
    if(err){
      pollQueueCallback (winston.makeError ('Error: could not generate xoauth token', {error : err, userEmail : userInfo.email}));
      return;
    }
    ...
    imapConnect.openMailbox (myConnection, function (err, mailbox) {
      if (err) {
        pollQueueCallback(winston.makeError ('Error: Could not open mailbox', {error : err, userEmail : userInfo.email}));
        return;
      }
