  ...
  sqsConnect.pollMailDownloadQueue(function (message, pollQueueCallback) {
    ...
    UserOnboardingStateModel.findOne ({'userId' : userInfo._id}, function (err, foundState) {
      if (err) {
        ...
        pollQueueCallback(); // message shouldn't be on queue onboarding complete
      }
      else if (foundState 
        && foundState.nodeId 
        && foundState.mikeyMailTS > Date.now() - constants.ONBOARDING_TIMESTAMP_UPDATE_INTERVAL * constants.ONBOARDING_TIMEESTAMP_RECLAIM_FACTOR) {
        ...
        // TODO: claim this onboarding state for yourself

      }
      else {
        winston.info ('User onboarding previously started', JSON.stringify(foundState));
        // TODO: claim this onboarding state for yourself
        claimOnboardingState (foundState);
        ...
        if (err) {
          ...
          imapConnect.closeMailbox (myConnection, function (err) {
            ...
            if (!setTimestampIntervalIds [userInfo._id]) {
              winston.doWarn ('No setTimestampIntervalIds for key', {userId : userInfo._id});
            }

            winston.doInfo ('done with initial indexing, \
              clearing interval to update mongo timestamp', 
              {userId : userInfo._id, intervalId : setTimestampIntervalIds [userInfo._id]});
