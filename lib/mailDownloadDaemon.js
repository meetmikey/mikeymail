...
exports.start = function () {
  ...
  sqsConnect.pollMailDownloadQueue(function (message, pollQueueCallback) {
    ...
    UserOnboardingStateModel.findOne ({'userId' : userInfo._id}, function (err, foundState) {
      ...
      winston.doInfo ('ISORestartTime', {ISORestartTime: ISORestartTime});
      ...
      if (err) {
        ...
        mailDownloadDaemon.scrapeMailbox (foundState._id, xoauth2gen, userInfo, pollQueueCallback, foundState.lastCompleted);
      }
      else {
        console.log ('mikeymail time', foundState.mikeyMailTS.toISOString());
        console.log ('restart time', ISORestartTime);
        console.log ('mikeymail ts < restart time', foundState.mikeyMailTS.toISOString() < ISORestartTime);
        winston.info ('User onboarding previously started and threshold not high enough to restart', {state : JSON.stringify(foundState),restartTime : ISORestartTime});
      }
    })
 ...
 */
exports.scrapeMailbox = function (onboardingStateId, xoauth2gen, userInfo, pollQueueCallback, lastCompleted) {
  ...
  xoauth2gen.getToken(function(err, token) {
    if(err){
      ...
      mailDownloadDaemon.clearTimeIntervalLoop (userInfo);
      return;
    }
...
exports.clearTimeIntervalLoop = function (userInfo) {
  winston.info ('clearTimeIntervalLoop for', {userEmail : userInfo.email, userId : userInfo._id});

  if (!setTimestampIntervalIds [userInfo._id]) {
    winston.doError ('No setTimestampIntervalIds for key', {userId : userInfo._id});
  }

  console.log ('before clearing', setTimestampIntervalIds);
  ...
  console.log ('after clearing', setTimestampIntervalIds);
