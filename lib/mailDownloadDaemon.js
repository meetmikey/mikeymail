...
console.log (constants.MAX_DOWNLOAD_JOBS)
...
exports.setListeningTimestampLoop = function (userId, callback) {
  ...
  var intervalId = setInterval (function () {
    ...
    UserOnboardingStateModel.update ({_id : userId, nodeId : myUniqueId}, 
      {$set : {'nodeId' : myUniqueId, 'listenTS' : Date.now()}}, 
      function (err, num) {
        if (err) {
...
exports.claimOnboardingState = function (userInfo){

  UserOnboardingStateModel.update ({_id : userInfo._id}, 
    {$set : {'nodeId' : myUniqueId, 'listenTS' : Date.now()}}, 
    function (err, num) {
      if (err) {
        winston.doError ('Error: could not set myUniqueId: ', {error : err});
