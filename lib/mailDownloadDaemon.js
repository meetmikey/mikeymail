...
exports.scrapeMailbox = function (xoauth2gen, userInfo, onboardingState, isFirstAttempt, pollQueueCallback) {
  ...
  xoauth2gen.getToken(function(err, token) {
    if(err){
      pollQueueCallback (winston.makeError ('Error: could not generate xoauth token: mailDownloadDaemon', {error : err, userEmail : userInfo.email}));
      
      if (err == 'invalid_grant') {
        //TODO: update token info for user in DB
      }
      return;
    }
