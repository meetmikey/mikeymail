...
exports.establishConnectionAndUpdate = function (user, xoauth2gen, pollQueueCallback) {
  xoauth2gen.getToken(function(err, token) {
    ...
    imapConnect.openMailbox (myConnection, function (err, mailbox) {
      ...
      if (err) {
        winston.doError ('Error: could not open mailbox', {error : err, userId : user._id, userEmail : user.email});
        winston.handleError (err);
        return;
      }

      winston.info ('Connection opened for user: ' + user.email)      
...
exports.updateMailbox = function (user, myConnection, mailbox, numNew, callback) {
  ...
  async.waterfall (operations, function (err) {
    ...
    if (err && !err.warning) {
      ...
      winston.info ('closing mailbox ' + user.email);
      // close the mailbox
      imapConnect.closeMailbox (myConnection, function (err) {
        if (err) {
          ...
          winston.info ('mailbox closed for user ' + user.email)
          callback ();
        }
      })
    }
