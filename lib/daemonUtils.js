...
exports.setUserImapDisabledError = function (isOnboarding, user, callback) {
  var duplicateError = false;
  if (user.imapDomainError) {
    duplicateError = true;
  }
  user.imapDomainError = true;
  ...
  user.save (function (err) {
    if (err) {
      ...
      if (!duplicateError) {
        sesUtils.sendImapDomainNotification (isOnboarding, user.email, callback);
      } else {
        winston.doInfo ('Duplicate imap disabled error, callback', {user : user.email, isOnboarding : isOnboarding});
        callback();
      }
    }
  });
...
exports.retrieveBatchRecurse = function (query, argDict, maxDateBatch, callback) {
  ...
  mongoQuery.exec (function (err, messages) {
    if (err) {
      ...
      var messagesToNotDownload = _.difference (messages, messagesToDownload);
      
      var msgToNotDownloadLength = messagesToNotDownload.length;
      var msgToDownloadLength = messagesToDownload.length;

      daemonUtils.markMessagesAsDone (messagesToNotDownload, function (err) {
        if (err) {
          callback (err);
        } else {
          winston.doInfo ('msgToDownloadLength', {len : msgToDownloadLength});
          winston.doInfo ('msgToNotDownloadLength', {len : msgToNotDownloadLength});
          ...
          if (!messages.length) {
            winston.doInfo ('nothing to download or not download');
            callback ();
            return;
          }
          else if (msgToNotDownloadLength > 0 && !messagesToDownloadLength) {
...
exports.markMessagesAsDone = function (msgs, callback) {

  if (msgs && !msgs.length) {
    return callback(); //nothing to do
  }

  var msgIds = _.map (msgs, function (msg) { return msg._id });
  MailModel.update ({_id : {$in : msgIds}}, 
    {$set : {mmDone : true}},
    {multi : true},
    function (err, num) {
      if (err) {
        ...
        callback (winston.makeError ('no records affected when marking done', {num : num, msgsIds : msgsIds}));
      } else {
        callback();
      }
    });
...
// always do this for resumes
exports.getUidRangeForDates = function getUidRangeForDates (argDict, callback) {
  var functionName = arguments.callee.name;
  winston.doInfo (functionName, {email : argDict.userEmail});
  ...
  if (argDict.isPremium) {
    ...
    imapRetrieve.getIdsOfMessagesInDateRange (argDict.myConnection, after, before, function (err, uids) {
      if (err) {
        callback (err);
      } else {
        argDict.uidArray = uids;
        callback (null, argDict);
      }
    });
  }
