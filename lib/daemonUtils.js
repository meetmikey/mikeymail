...
exports.setUserAllMailError = function (user, callback) {
  ...
  user.save (function (err) {
    if (err) {
      ...
      sesUtils.sendInternalNotificationEmail ('ALL MAIL doesn\'t exist for user ' + user.email, 'ALLMAIL missing error', 
        function (err) {
          callback (err);
        });
    }
...
exports.unSetUserAllMailError = function (user, callback) {
...
exports.accountLimitReached = function (argDict) {
  if (argDict.isPremium) {
    ...
    return (argDict.minProcessedDate.getTime() < argDict.minDateToProcess.getTime());
  }
}
...
exports.retrieveBatchRecurse = function (query, argDict, maxUidBatch, callback) {

  winston.doInfo ('retrieve batch recurse', {minUid : argDict.minUid, maxUid: maxUidBatch});

  var myConnection = argDict.myConnection;
  var userId = argDict.userId;
  var mongoQuery = MailModel.find(query);

  mongoQuery.where ('uid').gte (argDict.minUid);

  if (maxUidBatch !== '*') {
    mongoQuery.where('uid').lte(maxUidBatch);
  }

  mongoQuery.select ('uid gmDate');
    ...
    if (err) {
      ...
      if (daemonUtils.isLimitedAccount (argDict)) {
        messagesToDownload = _.filter (messages, function (msg) { 
          return msg.gmDate.getTime() >= argDict.minDateToProcess.getTime()
        });
      }
      ...
      if (!msgToDownloadLength) {
        return callback ();
      }

      // last element of array, minus 1 so we don't redo the current last msg
      var newMaxUid = messagesToDownload[msgToDownloadLength - 1].uid - 1
      ...
      imapRetrieve.getMessagesByUid (myConnection, userId, 
        ...
        if (err) {
          ...
          if (msgLength < constants.EMAIL_FETCH_BATCH_SIZE 
              || newMaxUid < 1 
              || daemonUtils.accountLimitReached (argDict)) {

            var logData = {
              userEmail : argDict.userEmail,
              userId : argDict.userId,
              minProcessedDate :  argDict.minProcessedDate,
              minDateToProcess :  argDict.minDateToProcess
            }

            winston.doInfo('retrieveBatchRecurse: Not retrieving emails anymore: no emails left to process for user', logData);
          ...
          // we can still get more messages
            ...
            daemonUtils.updateMinProccessedDateForUser (argDict.userId, argDict.minProcessedDate, function (err) {
              ...
              daemonUtils.retrieveBatchRecurse (query, argDict, newMaxUid, callback);
            });
          }
    ...
    MailModel.update ({userId : argDict.userId, uid : {$nin : results}}, 
      ...
      function (err, num) {
        if (err) {
          ...
          winston.doInfo(num + ' records marked isDeleted=true');
        }
      });
  })
}
...
exports.retrieveEmails = function retrieveEmails (argDict, callback) {
  ...
  daemonUtils.retrieveBatchRecurse (query, argDict, argDict.maxUid, function (err) {
    if (err) {
      callback (winston.makeError ('Error recursively getting attachments', {err : err}));
    }
    else {
      daemonUtils.updateStateAndCallback (functionName, argDict, callback);
    }
...
exports.markMinDateForUser = function markMinDateForUser (argDict, callback) {
  ...
  var userId = argDict.userId;
  MailModel.findOne ({userId : userId})
    .sort ('uid')
    .exec (function (err, minMail) {
      if (err) {
        callback (winston.makeMongoError (err));
      } else if (!minMail) {
        callback (winston.makeError ('could not find min mail for user', {userId : userId}));
      } else {
        UserModel.update ({_id : userId}, 
          {$set : {minMailDate : minMail.gmDate }},
          function (err, num) {
            if (err) {
              callback (winston.makeMongoError (err));
            } else if (num == 0) {
