      ...
      imapRetrieve.getMessagesByUid (myConnection, userId, 
        ...
        if (err) {
          ...
          if (msgLength < constants.EMAIL_FETCH_BATCH_SIZE || newMaxUid < 1) {
            winston.doInfo('retrieveBatchRecurse: Not retrieving emails anymore: no emails left to process for user', 
              {userEmail : argDict.userEmail, userId : argDict.userId});
            callback ();
          }
...
exports.lookupMailbox = function lookupMailbox (argDict, callback) {
  ...
  MailBox.findOne ({userId : argDict.userId}, function (err, foundMailbox) {
    if (err) {
      ...
      if (mailbox.uidnext == foundMailbox.uidNext && argDict.isInitialConnectUpdate) {
...
exports.retrieveEmails = function retrieveEmails (argDict, callback) {
  ...
  if (totalBandwith < constants.MAX_BANDWITH_TOTAL) {
    ...
    winston.doInfo('retrieveEmails: Not retrieving emails anymore \
      because bandwith limit exceeded', {totalBandwith : totalBandwith});
...
exports.updateMailbox = function updateMailbox (argDict, callback) {
  ...
  MailModel.find ({userId : argDict.userId, shardKey : mongoUtils.getShardKeyHash (argDict.userId)})
    .sort ('-uid')
    .select ('uid')
    .limit (1)
