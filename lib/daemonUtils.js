...
exports.retrieveBatchRecurse = function (query, argDict, maxUidBatch, isAttachment, callback) {
  ...
  console.log ('maxUid', maxUidBatch);
  ...
  if (isAttachment) {
    mongoQuery.limit (constants.EMAIL_FETCH_BATCH_SIZE_ATTACHMENT);
  }
  else {
  ...
  mongoQuery.exec (function (err, messages) {
    if (err) {
      ...
      winston.info ('message length', msgLength); 
      if (!msgLength) {
        return callback ();
      }
      ...
      imapRetrieve.getMessagesByUid (myConnection, userId, 
        ...
        if (err) {
          ...
          if ((isAttachment && msgLength < constants.EMAIL_FETCH_BATCH_SIZE_ATTACHMENT) ||
              (!isAttachment && msgLength < constants.EMAIL_FETCH_BATCH_SIZE) || 
              newMaxUid < 1) {
      ...
      daemonUtils.updateMailModel (uids, argDict.userId, 
        ...
        function (err, numAffected) {
          if (err) {
            ...
            winston.doInfo ('number of emails with attachments', {num : numAffected});
