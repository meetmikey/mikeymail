...
exports.updateMinProccessedDateForUser = function (userId, minProcessedDate, callback) {
  console.log ('minProcessedDate', minProcessedDate)
  ...
  mongoQuery.exec (function (err, messages) {
    if (err) {
      ...
      if (daemonUtils.isLimitedAccount (argDict)) {
        messagesToDownload = _.filter (messages, function (msg) { return msg.gmDate.getTime() >= argDict.minDateToProcess.getTime()});
      }

      console.log (messagesToDownload)

      var msgToDownloadLength = messagesToDownload.length;

      console.log ('msgToDownloadLength', msgToDownloadLength);
