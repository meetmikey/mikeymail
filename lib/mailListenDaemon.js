...
//TODO: find more error states where we need to call this...
exports.clearTimeIntervalLoop = function (userId) {
  winston.doInfo ('clearTimeIntervalLoop for user', {userId : userId});
  
  if (!setTimestampIntervalIds [userId]) {
    winston.doError ('No setTimestampIntervalIds for key', {userId : userId});    
  }

  clearInterval (setTimestampIntervalIds [userId]);
  delete setTimestampIntervalIds [userId];
