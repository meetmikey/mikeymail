...
exports.connectAndUpdate = function (user, xoauth2gen) {
  ...
  winston.doInfo ('connectAndListen for user', {user : user});
      ...
      if (err) {
        ...
        if ( errorType === mikeymailConstants.ERROR_TYPE_ALL_MAIL_DOESNT_EXIST ) {
          if (!user.allMailError) {
            daemonUtils.setUserAllMailError (user, function (err) {
              if (err) { winston.handleError (err); }
            });
          }
        } else if (errorType === 'timeout') {
          winston.doWarn ('mailListenDaemon: imap connect warning - timeout', {userEmail : userEmail});
        } else {
          winston.handleError (err);
        }
