  ...
  xoauth2gen.getToken(function(err, token) {
    ...
    imapConnect.openMailbox (myConnection, function (err, mailbox) {
      ...
      myConnection.on ("mail", function (num) {
        ...
        mailUpdateDaemon.updateMailbox (userInfo, myConnection, mailbox, num, keepMailboxOpen=true, function (err, warn) {
          if (err) {
            winston.doError ('Error updating mailbox on listen daemon', {err :err, userId: userInfo._id});
          }
          else if (warn) {
            winston.doInfo ('No new mail to update for user', {warn : warn});
          }
          else {
