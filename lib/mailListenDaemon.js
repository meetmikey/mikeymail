  ...
  xoauth2gen.getToken(function(err, token) {
    ...
    imapConnect.openMailbox (myConnection, function (err, mailbox) {
      ...
      mailUpdateDaemon.updateMailbox (userInfo, myConnection, mailbox, numNew, keepMailboxOpen=true, function (err) {
        if (err) {
          winston.doWarn ('Error updating mailbox on initial connection of listen daemon', {err :err, userId: userInfo._id});
        }
      ...
      myConnection.on ("mail", function (num) {
        ...
        mailUpdateDaemon.updateMailbox (userInfo, myConnection, mailbox, num, keepMailboxOpen=true, function (err, numMessages) {
          if (err) {
            ...
            winston.doInfo ('No new mail to update for user', {userId : userInfo._id, userEmail : userInfo.email});
          }
        });
      });
