...
exports.openMailbox = function (imap, userEmail, cb) {
  ...
  imap.once('error', function(err) {
    if (callbackCalled) {
      ...
      var winstonError = winston.makeError ('imap connect error', {err : err, email : userEmail});
      if (err && err.message && err.message.indexOf ('IMAP access is disabled for your domain') != -1) {
        winston.doWarn ('imap access disabled for domain');
        winston.setErrorType (winstonError, mikeymailConstants.ERROR_TYPE_IMAP_DOMAIN_ERROR);
      } else if (err && err.level) {
        winston.setErrorType( winstonError,  err.level);
      } else if (err && err.source) {
        winston.setErrorType (winstonError, err.source);
      }
